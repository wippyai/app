...
local function handler()
    ...
    if type(data.disabled) ~= "boolean" then
        res:set_status(http.STATUS.BAD_REQUEST)
        res:write_json({
            success = false,
            error = "disabled field must be boolean"
        })
        return
    end
    ...
    if not response then
        ...
        res:write_json({
            success = false,
            error = "Failed to update key via OpenRouter API: " .. err
        })
        return
    end
    ...
    local db_result, db_err = keys_repo.set_disabled(key_id, data.disabled)
    if not db_result then
        res:set_status(http.STATUS.INTERNAL_ERROR)
        res:write_json({
            success = false,
            error = "Failed to update key in database: " .. db_err
        })
        return
    end
