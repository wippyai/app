...
local VALIDATION_ERRORS = {
    ...
    ACCESS_DENIED = "Insufficient access to Tempo API"
}

local tempo_internal = {}

-- Get component_id from context with validation
function tempo_internal.get_component_id()
...
function tempo_internal.validate_access(component_id, access_level)
...
function tempo_internal.get_credentials(component_id)
    ...
    if not creds_result.credentials then
        return nil, "No Tempo credentials configured"
    end
...
function tempo_internal.build_auth_headers(credentials, additional_headers)
    local headers = {
        ["Accept"] = "application/json",
        ["Content-Type"] = "application/json"
    }

    -- Tempo uses Bearer token authentication
...
function tempo_internal.build_query_string(query_params)
...
function tempo_internal.build_url(credentials, path, query_params)
    ...
    local query_string = tempo_internal.build_query_string(query_params)
    return server_url .. path .. query_string, nil
end

-- Parse JSON response if possible
function tempo_internal.parse_response(response)
