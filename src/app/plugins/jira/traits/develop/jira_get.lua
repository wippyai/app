...
local function handle(args)
    ...
    -- Open Jira connection (will auto-discover if component_id is nil)
    local service, err = jira.open(component_id)
    if err then
        return nil, "Error connecting to Jira: " .. err
    end

    if not service then
        return nil, "No Jira connection available"
    end
    ...
    if result.success then
        return "Jira GET " .. args.path .. " - Success\n\n" .. (result.formatted or "No data"), nil
    else
        local error_msg = "Jira GET " .. args.path .. " - Failed (Status: " .. result.status_code .. "): " .. (result.error_message or "Unknown error")
