...
local function handle(request_dto)
    ...
    -- Build full URL
    local full_url, url_err = jira_internal.build_url(credentials, request_dto.path, request_dto.query)
    if url_err then
        return { status_code = 500, body = { error = url_err } }
    end

    -- Build headers
    local headers = jira_internal.build_auth_headers(credentials)
    ...
    return jira_internal.parse_response(response)
end
