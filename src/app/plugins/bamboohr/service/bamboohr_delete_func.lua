...
local function handle(request_dto)
    ...
    -- Validate WRITE access for DELETE requests
    local access_level, access_err = bamboohr_internal.validate_access(component_id, component.ACCESS.WRITE)
    if access_err then
    ...
    -- Build full URL
    local full_url, url_err = bamboohr_internal.build_url(credentials, request_dto.path, request_dto.query)
    if url_err then
        return { status_code = 500, body = { error = url_err } }
    end

    -- Build headers
    local headers = bamboohr_internal.build_auth_headers(credentials, request_dto.headers)

    -- Make HTTP request
    local response, http_err = http_client.delete(full_url, {
        headers = headers,
        timeout = 30
    })
    ...
    return bamboohr_internal.parse_response(response)
end
